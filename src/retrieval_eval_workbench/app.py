...
<p><textarea id='query' rows='3'>What happens to a case if no routing rule matches?</textarea><br><button onclick='run()'>Run query</button> <button onclick='showResults()'>Show latest benchmark</button></p><div id='output' class='card'>Run a query to inspect retrieved documents, answer, and line-level citation.</div>
<script>async function run(){let payload={query:query.value,strategy:strategy.value,top_k:+topk.value};let r=await fetch('/api/query',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});let d=await r.json();if(!r.ok){output.textContent=d.detail;return}let c=d.citations.map(x=>`<li><b>${x.title}</b>, line ${x.line_start}: “${x.quote}”</li>`).join('')||'<li>No citation: corpus evidence was not found.</li>';let h=d.hits.map(x=>`<li>${x.title} <span class='muted'>(${x.score})</span></li>`).join('');output.innerHTML=`<h2>${d.answerable?'Grounded extract':'No supported answer'}</h2><p>${d.answer}</p><h3>Citations</h3><ul>${c}</ul><h3>Retrieved documents</h3><ol>${h}</ol>`}async function showResults(){let r=await fetch('/api/results');let d=await r.json();if(!r.ok){output.textContent=d.detail;return}let cards=Object.entries(d.measured_results.results).map(([name,x])=>`<li><b>${name}</b>: Recall@3 ${x.metrics.recall_at_3.toFixed(2)}, MRR@3 ${x.metrics.mrr_at_3.toFixed(2)}, citation correctness ${x.metrics.citation_correct_rate.toFixed(2)}, grounded answers ${x.metrics.grounded_answer_rate.toFixed(2)}, unanswerable correctness ${x.metrics.unanswerable_correct_rate.toFixed(2)}</li>`).join('');output.innerHTML=`<h2>Held-out benchmark</h2><ul>${cards}</ul><p class='muted'>Source revision: ${d.source_revision}; regression: ${d.measured_results.regression.passed?'passed':'failed'}.</p>`}</script></body></html>"""
